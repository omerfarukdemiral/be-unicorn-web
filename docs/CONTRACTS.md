...
- **Yerleşim (docs/LAYOUT.md):** üç sabit yüzey + tek bildirim şeridi, her bilgi tek yerde.
  - **Üst bar** (`layout/TopBar.tsx`, tam genişlik h56; telefonda 2 satır): A = aşama · tarih (nötr ay halkası) · `derived.stageProgress` · tur çipi "Tur 3/11 hf" ya da "Tur başlat" (turun tek evi); B = Kasa (`cashFlow().usable` + "net −$X/gün"), Runway, Kullanıcı, Moral + en fazla 2 sabitlenmiş metrik (`ui.pinnedMetrics`, `widgets.tsx › usePinnedMetrics/visiblePins/PinnedMetric`); C = hız (`layout/SpeedControl.tsx`, zaman durumu etiketi burada) · zoom · (TR ≥1536) · ayarlar. Kök `data-scene-top`.
  - **Alt bar** (`layout/BottomBar.tsx › BottomStack`): solda enerji + etiketli kurucu aksiyonları (`layout/FounderBar.tsx`), sağda sekmeler (`Dock.tsx › DOCK_TABS`: Mağaza M · Ekip E · Projeler P · Büyüme B · Metrikler G · Kazanımlar K). Hemen üstünde **bildirim şeridi** (`layout/NotificationStrip.tsx`, kurallar `layout/stripRules.ts`): iflas/runway<3 (P0) › yerleştirme (P1) › hata, ay fişi, sürüm, karar sonucu, hedef, tur, yeni metrik, aktivite (P2) › sıradaki adım (P3); sağda ufuk özeti + ⌃ popover (yaklaşanlar + son olaylar). Sarmalayıcı `data-scene-bottom`.
  - **Metrikler** sekmesi (`panels/MetricsPanel.tsx`): açılan ikincil metrikler (Para / Büyüme / Ekip / Yol), "Üst bara sabitle" (en fazla 2, 3.'sü en eskiyi atar; `pinMetric/unpinMetric`, kalıcı `be-unicorn:ui`), yeni metrik rozeti (`markMetricsSeen`). Metrikler duraklatmaz.
  - **Para tek tanım:** `src/ui/cashflow.ts › cashFlow(state)` (Kasa = kullanılabilir, "net/gün" = (Gelir − Yakıt) ÷ 30, Yakıt = aylık toplam gider, Kâr tahmini = Gelir − Yakıt).
  - **Kırmızı** yalnız gerçek tehlike: runway < 3 ay, kullanılabilir kasa < 0 / iflas sayacı, kaçan maaş, moral < 28. Hız rengi yalnız hız kontrolünde + ince `ScreenFrame`; duraklıyken çerçeve nötr gri ve kesik.
- **Tek panel** (`ui.panel`, `RightPanel.tsx`): Mağaza · Ekip · Projeler · Büyüme (tur bölümü dahil) · Defter (kavram kartı dahil) · sahne detayı · karar + yansıma · ayarlar aynı sağ panelde açılır ve birbirinin yerini alır; masaüstünde sağda üst barla alt bar arasında 400px (1280 altı 360px) sütun (`data-scene-right`), mobilde alt barın sekme satırının 8px üstünde tek sheet (`data-scene-sheet`). Tek seviye geri (`ui.panelBack`). Alt dock yalnızca sekme çubuğu (`togglePanel`). Boş zemine tık (`select(null)`) detay / hedefli mağazadan geri döner (geçmiş yoksa kapatır). Açık bir slot detayının eşyası satılırsa panel o slota hedefli Mağaza'ya, taşınırsa yeni slotun detayına geçer (store `dispatch`). `ui.sceneInset` ({top,right,bottom} px) tek yerden yazılır: `layout/useSceneInset.ts` üst bar, şerit + alt bar, panel ve sheet'i (`data-scene-*`, transform hariç yerleşim kutusu) ölçer; render `CameraRig` ofisi, `BubbleAnchor` balonları, `BubbleTray` / `StartCall` kendilerini bu alanda yerleştirir. Telefonda şerit popover'ı da tek-açık kuralına uyar (`useExclusiveExpander`).
...
- Aktivite: `state.activity` + `ACTIVITY_TEXT[kind]` içindeki `{param}` doldurulur; şeritte (seçili türler) ve popover geçmişinde görünür.
- Hata geri bildirimi: `ui.lastError` şeritte kısa satır (P2).
- Mobil (< 768px): üst bar 2 satır (aşama + hız / 4 gösterge), alt bar 2 satır (aksiyon ikonları / kaydırılabilir sekmeler), şerit tek satır, panel alttan sheet (açıkken aksiyon satırı gizlenir), dokunma hedefleri ≥ 44px, `safe-top/safe-bottom/safe-x` yardımcıları.
